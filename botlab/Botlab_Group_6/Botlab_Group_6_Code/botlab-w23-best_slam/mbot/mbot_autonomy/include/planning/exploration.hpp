...
public:
    ...
    */
    ...
    void handleConfirmation(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const mbot_lcm_msgs::message_received_t* confirm);
...
private:
    ...
    mbot_lcm_msgs::pose_xyt_t currentPose_;  // Robot pose to use for computing new paths
    mbot_lcm_msgs::pose_xyt_t currentGoal_; 
    ...
    mbot_lcm_msgs::pose_xyt_t homePose_;  // Pose of the robot when it is home, i.e. the initial pose before exploration begins

    mbot_lcm_msgs::robot_path_t currentPath_;  // Current path being followed to a frontier or other target, like the home or key poses
    std::vector<frontier_t> frontiers_; // Current frontiers in the map
    
    // Data coming in from other modules -- used by LCM thread
    mbot_lcm_msgs::pose_xyt_t incomingPose_;  // Temporary storage for the most recently received pose until needed by explore thread
