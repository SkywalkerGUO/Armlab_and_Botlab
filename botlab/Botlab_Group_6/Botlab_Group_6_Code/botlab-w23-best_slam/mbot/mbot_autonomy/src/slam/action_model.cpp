...
bool ActionModel::updateAction(const mbot_lcm_msgs::pose_xyt_t& odometry)
{
    ...
    //check motion, calculate deltas an calculate standard deviations

    //Compare odometry with previousPose_ to see if it moved -- CHECK MOTION
    moved_ = 0;
    ...
    //Calculate deltas
    dx_ = odometry.x - previousOdometry_.x;
    dy_ = odometry.y - previousOdometry_.y;
    dtheta_ = odometry.theta - previousOdometry_.theta;

    trans_ = std::sqrt(dx_ * dx_ + dy_ * dy_);
    rot1_ = angle_diff(std::atan2(dy_, dx_), previousOdometry_.theta);
    float direction = 1.0;
