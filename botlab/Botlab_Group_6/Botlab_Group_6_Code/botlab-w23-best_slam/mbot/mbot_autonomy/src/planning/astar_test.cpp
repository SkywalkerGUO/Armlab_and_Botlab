...
mbot_lcm_msgs::robot_path_t timed_find_path(const mbot_lcm_msgs::pose_xyt_t& start, 
                             const mbot_lcm_msgs::pose_xyt_t& end, 
                             const MotionPlanner& planner, 
                             const std::string& testName);

bool is_valid_path(const mbot_lcm_msgs::robot_path_t& path, double robotRadius, const OccupancyGrid& map);
...
int main(int argc, char** argv)
{
    ...
    getopt_add_bool(gopt, '\0', useGuiArg, true, "Indicates whether ./botgui is being used for visualization."
                    " Set this to 1 or true to send lcm messages with pose and grid data for visualization."
                    " './botgui' has to be running in parallel.");
    getopt_add_bool(gopt, '\0', animatePathArg, true, "If this flag is set to true when visualizing, "
...
bool is_valid_path(const mbot_lcm_msgs::robot_path_t& path, double robotRadius, const OccupancyGrid& map)
{
    ...
    mbot_lcm_msgs::robot_path_t printpath;
