...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    ...
    double distToGoal = distance_between_points(Point<float>(currentGoal_.x, currentGoal_.y), 
                                                Point<float>(currentPose_.x, currentPose_.y));

    std::cout << "Dist to goal: " << distToGoal << std::endl;
    frontier_processing_t front_processing_ = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);
    //std::cout << "Frontiers: " << frontiers_ << std::endl;
    //This will make new path
    if (!frontiers_.empty() && distToGoal < 3.0){ //TODO: this value??
        //We path plan again when we are close to the end of reaching the frontier
        currentPath_ = front_processing_.path_selected;
        std::cout << "planned path to frontier (exploration)\n";
        //currentPath_ = currentPath_.path.erase(currentPath_.path.end()-3, currentPath_.path.end());
        if (currentPath_.path.size() > 0){
            // currentPath_.path.erase(currentPath_.path.begin(), currentPath_.path.begin() + 1);
            // currentPath_.path_length -= 1;
            currentGoal_ = currentPath_.path[currentPath_.path_length - 1]; // last node in path
            std::cout << "current goal is: " << currentGoal_.x << ", " << currentGoal_.y  << std::endl;
        }
    }
    else{
        std::cout << "frontiers empty or dist to goal too far \n";
    }
    ...
    //printf("Num unreachable frontiers: %d\n", front_processing_.num_unreachable_frontiers);
...
int8_t Exploration::executeReturningHome(bool initialize)
{
    ...
    */
    ...
    double distToHome = distance_between_points(Point<float>(homePose_.x, homePose_.y), 
                                                Point<float>(currentPose_.x, currentPose_.y));
    bool reachedHome = (distToHome <= kReachedPositionThreshold);                                           
    bool goingHome = currentPath_.path.size() > 1;
