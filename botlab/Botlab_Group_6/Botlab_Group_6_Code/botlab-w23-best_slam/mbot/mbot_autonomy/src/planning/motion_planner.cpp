...
bool MotionPlanner::isPathSafe(const mbot_lcm_msgs::robot_path_t& path) const
{
    ...
    // check if path goes through any occupied cells, goes out of the map, goes too close to goal?
    // for(auto & p : path.path) {
    //     if(distances_.isCellInGrid(p.x, p.y))
    //     {
    //         // And is far enough from obstacles that the robot can physically occupy the space
    //         // Add an extra cell to account for discretization error and make motion a little safer by not trying to
    //         // completely snuggle up against the walls in the motion plan
    //         return distances_(p.x, p.y) > params_.robotRadius;
    //     }
    // }
    for(int i = 0; i < path.path_length; i++) {
        if(distances_.isCellInGrid(path.path[i].x, path.path[i].y))
        {
            // And is far enough from obstacles that the robot can physically occupy the space
            // Add an extra cell to account for discretization error and make motion a little safer by not trying to
            // completely snuggle up against the walls in the motion plan
            return distances_(path.path[i].x, path.path[i].y) > params_.robotRadius;
        }
    }
...
{
    searchParams_.minDistanceToObstacle = 0.15; // params_.robotRadius;
    searchParams_.maxDistanceWithCost = 10.0 * searchParams_.minDistanceToObstacle;
    searchParams_.distanceCostExponent = 1.0; // can modify these values
}
