...
private:
    float pid[3] = {1.0, 4.0, -0.1}; //kp, ka, kb //kd ka kb NOT PID values kb is always negative
    //1, 4, -0.1 for slow speed
...
public:
    ...
    virtual mbot_lcm_msgs::mbot_motor_command_t get_command(const mbot_lcm_msgs::pose_xyt_t& pose, const mbot_lcm_msgs::pose_xyt_t& target) override
    {
        ...
        if (fabs(alpha) > 0.2) //0.3 for slow speed 
        {
            fwd_vel = 0;
        }

        // printf("%f,%f\n", fwd_vel, turn_vel);
        printf("alpha: %f, pose_theta %f diff %f fwd_vel %f turn_vel %f \n", alpha, pose.theta, diff, fwd_vel, turn_vel);
        return {0, fwd_vel, turn_vel};
    }
...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            // Fwd vel
            float MAX_VEL = 0.8;
            if (cmd.trans_v > MAX_VEL) cmd.trans_v = MAX_VEL;
            else if (cmd.trans_v < -MAX_VEL) cmd.trans_v = -MAX_VEL;

            // Angular vel
            float max_ang_vel = M_PI/4.0; //M_PI * 2.0 / 3.0;
